# R code without documentation for running our code
# Towards a Structured Evaluation Methodology for Artificial Intelligence Technology (SEMAIT)
...
sdms_df$dataset_metric = as.factor(paste(as.character(sdms_df$dataset), as.character(sdms_df$metric), sep="_"))
sdms_df$metric_dataset = as.factor(paste(as.character(sdms_df$metric), as.character(sdms_df$dataset), sep="_"))
...
grid.arrange(accuracy_plot, auc_plot, dpr_plot, f1_plot, 
             precision_plot, recall_plot, nrow = 2)
...
kable(bc_mar_df[, c("treatment_1", "frac_1_better_or_equal_than_2")], caption="Block Comparison Ranking", digits=4)
...
system_rank_df <- sdms_df %>% 
  ...
  group_by(dataset, metric) %>% mutate(rank=rank(-score)) %>% as.data.frame()
metric_rank_df <- sdms_df %>% 
  arrange(dataset, system, score) %>% 
  group_by(dataset, system) %>% mutate(rank=rank(-score)) %>% as.data.frame()
dataset_rank_df <- sdms_df %>% 
  arrange(system, metric, score) %>% 
  group_by(system, metric) %>% mutate(rank=rank(-score)) %>% as.data.frame()

mizr_tile_table_plot(system_rank_df, c("dataset", "metric"), "system", "rank", use_margins = TRUE, tile_text_size = rel(3.6))

dataset_list <- c("adult", "hmda", "titanic")
adult_df <- system_rank_df[system_rank_df$dataset == "adult", ]
adult_plot <- mizr_tile_table_plot(adult_df, "metric", "system", "rank", use_margins = TRUE, tile_text_size = rel(3.6))
adult_plot <- adult_plot + ggtitle("system Rank for each Metric of \nadult Dataset")
hmda_df <- system_rank_df[system_rank_df$dataset == "hmda", ]
hmda_plot <- mizr_tile_table_plot(hmda_df, "metric", "system", "rank", use_margins = TRUE, tile_text_size = rel(3.6))
hmda_plot <- hmda_plot + ggtitle("system Rank for each Metric of \nhmda Dataset")
titanic_df <- system_rank_df[system_rank_df$dataset == "titanic", ]
titanic_plot <- mizr_tile_table_plot(titanic_df, "metric", "system", "rank", use_margins = TRUE, tile_text_size = rel(3.6))
titanic_plot <- titanic_plot + ggtitle("system Rank for each Metric of \ntitanic Dataset")
# Now use a grid or a wrap to place all 7 plots on one page
grid.arrange(adult_plot, hmda_plot, titanic_plot, nrow = 1)

mrank_mar_df <- tile_table_compute_df(system_rank_df, c("dataset", "metric"), "system", "rank", use_margins = TRUE)
mr_df <- mrank_mar_df[mrank_mar_df$metric == "(all)" & mrank_mar_df$dataset == "(all)" & mrank_mar_df$system != "(all)" , ]
mr_df <- mr_df[order(mr_df$rank), ]
kable(mr_df[, c("system", "rank")], caption="Ranking by Mean Rank on Metrics and Datasets", digits=4)

mizr_tile_table_plot(metric_rank_df, c("dataset", "system"), "metric", "rank", use_margins = TRUE, tile_text_size = rel(3.6))
dataset_list <- c("adult", "hmda", "titanic")
adult_df <- metric_rank_df[metric_rank_df$dataset == "adult", ]
adult_plot <- mizr_tile_table_plot(adult_df, "system", "metric", "rank", use_margins = TRUE, tile_text_size = rel(3.6))
adult_plot <- adult_plot + ggtitle("Metric Rank for each system of \nadult Dataset")
hmda_df <- metric_rank_df[metric_rank_df$dataset == "hmda", ]
hmda_plot <- mizr_tile_table_plot(hmda_df, "system", "metric", "rank", use_margins = TRUE, tile_text_size = rel(3.6))
hmda_plot <- hmda_plot + ggtitle("Metric Rank for each system of \nhmda Dataset")
titanic_df <- metric_rank_df[metric_rank_df$dataset == "titanic", ]
titanic_plot <- mizr_tile_table_plot(titanic_df, "system", "metric", "rank", use_margins = TRUE, tile_text_size = rel(3.6))
titanic_plot <- titanic_plot + ggtitle("Metric Rank for each system of \ntitanic Dataset")
# Now use a grid or a wrap to place all 7 plots on one page
grid.arrange(adult_plot, hmda_plot, titanic_plot, nrow = 1)
...
mizr_tile_table_plot(dataset_rank_df, c("system", "metric"), "dataset", "rank", use_margins = TRUE, tile_text_size = rel(3.6))

mizr_interact_compare_plot(sdms_df, c("dataset", "metric"), "system", "score") + scale_shape_manual(values=c(1,2,16,17,18,19,3,4,5))
mizr_interact_compare_plot(sdms_df, c("metric_dataset"), "system", "score") + scale_shape_manual(values=c(1,2,16,17,18,19,3,4,5))
...
mizr_tile_table_plot(sdms_df, c("metric", "dataset"), c("system"),  response_var, 
                     midpoint=median(sdms_df$score), tile_text_size = rel(2.5)) + theme(legend.position="bottom", legend.text = element_text(angle = 90, vjust=0.5))
mizr_tile_table_plot(sdms_df, c("metric", "dataset"), c("system"),  response_var, 
                     midpoint=median(sdms_df$score), tile_text_size = rel(2.5), use_margins=TRUE) + theme(legend.position="bottom", legend.text = element_text(angle = 90, vjust=0.5))
